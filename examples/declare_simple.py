...
def execute_script():
    ...
    declare_model: "dict[str, dict[Any, dict[str, int]]]" = pm4py.discover_declare(log)
    print(declare_model)

    # prints a textual abstraction of the declare model that can be provided to GPT
    text_abstr: "str" = pm4py.llm.abstract_declare(declare_model)
    print(text_abstr)

    # provides conformance checking based on the DECLARE model
    conf_result: "list[dict[str, Any]]" = pm4py.conformance_declare(log, declare_model, return_diagnostics_dataframe=True)
    print(conf_result)
